...
_QS_ST_showMOS_range = 1;										// NUMBER. Range in distance to show MOS on the map. Default 3500.
...
_QS_ST_iconUpdatePulseDelay = 0.1;								// NUMBER. How often should location of unit on the MAP be updated? 0 = as fast as possible, else if > 0 then it = time in seconds. Default 0.
...
_QS_ST_iconSize_Man = 24;										// NUMBER. Icon Size by Vehicle Type. Man/Units. Default = 22
...
_QS_ST_iconSize_Air = 31;										// NUMBER. Icon Size by Vehicle Type. Air vehicles. Default = 24
_QS_ST_iconSize_StaticWeapon = 24;								// NUMBER. Icon Size by Vehicle Type. Static Weapon (Mortar, remote designator, HMG/GMG. Default = 22
...
if (_QS_ST_X select 2) then {
	...
	[_QS_ST_X] spawn {
		...
		if (!(_showCivilianGroups)) then {
			private _nil = _sides deleteAt 3;
		};
